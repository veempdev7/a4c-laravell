...
PIE.showParents = function(el) {
	var showed = [];
	var parent = el;
	while (parent) {
		if (parent.currentStyle && parent.currentStyle.display === "none") {
			parent.style.display = "block";
			showed.push({el : parent, style : parent.style.display === "none"});
		}
...
PIE.hideEls = function(els) {
	for(var i = 0; i < els.length; i++) {
		if (els[i].style) {
			els[i].el.style.display = "none";
		} else {
			els[i].el.style.display = "";
		}
	}
};
...
PIE.BackgroundStyleInfo = PIE.StyleInfoBase.newStyleInfo( {
    ...
    getBgAreaSize: function( bgOrigin, boundsInfo, borderInfo, paddingInfo ) {
		...
		var bounds = boundsInfo.getBounds(),
            w = bounds.w,
            h = bounds.h,
            borders, paddings;
...
PIE.Element = (function() {
    ...
    var wrappers = {},
        ...
        ignorePropertyNames = { 'background':1, 'bgColor':1, 'display': 1, 'style.display' : 1},
        classNameRegExes = {},
        dummyArray = [];
    ...
    function Element( el ) {
		...
		me.mouseLeft = mouseLeft;
		me.mouseEntered = mouseEntered;
    }
